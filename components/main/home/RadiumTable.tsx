...
import { 
  ArrowUpIcon, ArrowDownIcon, TrendingUpIcon, TrendingDownIcon, Search, ChevronLeft, ChevronRight,
  BarChart2, DollarSign, Activity, Percent, RefreshCw, Settings, Moon, Sun, HelpCircle, Bell
} from 'lucide-react';
...
const RadiumDashboard: React.FC<RadiumDashboardProps> = ({ data, handleClick }) => {
  ...
  const [changes, setChanges] = useState<Record<string, Partial<Record<keyof RadiumPool, number>>>>({});
  ...
  useEffect(() => {
    const newChanges: Record<string, Partial<Record<keyof RadiumPool, number>>> = {};
    ...
    data.forEach(pool => {
      ...
      if (prev) {
        newChanges[pool.id] = {
          ...
          'day.volume': calculatePercentageChange(prev.day.volume, pool.day.volume),
          'day.volumeFee': calculatePercentageChange(prev.day.volumeFee, pool.day.volumeFee),
  ...
  const filteredAndSortedData = useMemo(() => {
    return data
      .filter(pool => 
        (showLowLiquidity || pool.tvl >= 5000) &&
        (pool.mintA.symbol.toLowerCase().includes(searchTerm.toLowerCase()) ||
         pool.mintB.symbol.toLowerCase().includes(searchTerm.toLowerCase()))
      )
      .sort((a, b) => {
  ...
  }, [data, sortConfig, searchTerm, showLowLiquidity]);
  ...
  const getSortIndicator = (key: keyof RadiumPool) => {
    ...
    return sortConfig.direction === 'ascending' ? <ArrowUpIcon className="inline w-4 h-4 ml-1" /> : <ArrowDownIcon className="inline w-4 h-4 ml-1" />;
  };
  ...
  const totalLiquidity = useMemo(() => data.reduce((sum, pool) => sum + pool.tvl, 0), [data]);
  const totalVolume = useMemo(() => data.reduce((sum, pool) => sum + pool.day.volume, 0), [data]);
  const averageAPR = useMemo(() => data.reduce((sum, pool) => sum + pool.day.apr, 0) / data.length, [data]);

  return (
    <div className={`${isDarkMode ? 'bg-gray-900' : 'bg-gray-100'} min-h-screen p-6`}>
      <header className="flex justify-between items-center mb-8">
        <h1 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>Radium Pool Dashboard</h1>
        <div className="flex items-center space-x-4">
          ...
          <button className="p-2 rounded-full hover:bg-gray-700">
            <HelpCircle className={isDarkMode ? 'text-white' : 'text-gray-800'} />
          </button>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className={`p-6 rounded-lg shadow-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
          <div className="flex items-center justify-between">
            <h2 className={`text-xl font-semibold ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>Total Liquidity</h2>
        ...
        <div className={`p-6 rounded-lg shadow-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
          <div className="flex items-center justify-between">
            <h2 className={`text-xl font-semibold ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>24h Volume</h2>
            <BarChart2 className={`w-8 h-8 ${isDarkMode ? 'text-green-400' : 'text-green-600'}`} />
          </div>
          <p className={`text-3xl font-bold mt-2 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
        ...
        <div className={`p-6 rounded-lg shadow-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
          <div className="flex items-center justify-between">
            <h2 className={`text-xl font-semibold ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>Average APR</h2>
            <Percent className={`w-8 h-8 ${isDarkMode ? 'text-purple-400' : 'text-purple-600'}`} />
          </div>
          <p className={`text-3xl font-bold mt-2 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>
      ...
      <div className={`bg-${isDarkMode ? 'gray-800' : 'white'} p-6 rounded-lg shadow-xl`}>
        <div className="flex justify-between items-center mb-4">
          <div className="relative">
            <input
              type="text"
              placeholder="Search pools..."
              className={`${isDarkMode ? 'bg-gray-700 text-white' : 'bg-gray-100 text-gray-800'} px-4 py-2 pr-10 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500`}
              value={searchTerm}
          ...
          <div className="flex items-center space-x-4">
            <label className="flex items-center cursor-pointer">
              ...
              />
              <span className={`ml-2 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}>Show low liquidity pools</span>
            </label>
            <button className={`p-2 rounded-full ${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-200'}`}>
              <RefreshCw className={isDarkMode ? 'text-white' : 'text-gray-800'} />
            </button>
          </div>
        </div>
        <div className="overflow-x-auto shadow-lg rounded-lg">
          <table className={`min-w-full divide-y ${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
            <thead className={isDarkMode ? 'bg-gray-800' : 'bg-gray-50'}>
              <tr>
                {['Pool', 'Liquidity', 'Volume 24H', 'Fees 24H', 'APR 24H', 'Actions'].map((header) => (
                  <th
                    key={header}
                    className={`px-6 py-3 text-left text-xs font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} uppercase tracking-wider cursor-pointer transition-all duration-200 hover:bg-gray-700 hover:text-white`}
                    onClick={() => requestSort(header.toLowerCase().replace(' ', '') as keyof RadiumPool)}
                  >
                    {header} {getSortIndicator(header.toLowerCase().replace(' ', '') as keyof RadiumPool)}
                  </th>
                ))}
              </tr>
            </thead>
              ...
              {paginatedData.map((pool) => (
                <tr 
                  ...
                  className={`transition-all duration-300 ease-in-out ${
                    animatedRows.includes(pool.id) ? 'bg-blue-900 bg-opacity-30' : ''
                  } ${hoveredRow === pool.id ? (isDarkMode ? 'bg-gray-700' : 'bg-gray-100') : ''}`}
        ...
        <div className="mt-4 flex justify-between items-center">
          ...
          <div className="flex space-x-2">
            <button
              ...
              className={`px-3 py-1 ${isDarkMode ? 'bg-gray-700 text-white' : 'bg-gray-200 text-gray-800'} rounded-md disabled:opacity-50`}
            >
              <ChevronLeft size={20} />
            </button>
            <button
